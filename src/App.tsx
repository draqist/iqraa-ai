...
export default function App() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!process.env.API_KEY) {
      alert("API Key missing. Cannot process PDF.");
      return;
    }
    ...
    try {
      ...
      setProcessingStatus('AI is analyzing text structure (this may take a moment)...');
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            {
              text: `Analyze this PDF document which contains an Islamic text (Mutn). 
              Extract the content into a structured JSON format. 
              1. Identify the Title (Arabic and English/Transliterated).
              2. Identify the Author.
              3. Extract all verses/lines in order.
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              title_en: { type: Type.STRING, description: "English or Transliterated title" },
              title_ar: { type: Type.STRING, description: "Arabic title" },
              ...
              nodes: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    text_english: { type: Type.STRING, description: "Translation if available, else empty string" },
                    node_type: { type: Type.STRING, enum: ['verse', 'line'] }
                  }
                }
              }
            },
            required: ["title_en", "title_ar", "nodes"]
          }
        }
      });

      setProcessingStatus('Finalizing book...');
      ...
      if (!extractedData.nodes || extractedData.nodes.length === 0) {
        throw new Error("Could not extract text nodes from PDF");
      }

      // Map to our internal Book type
      const newBook: Book = {
        id: `custom-${Date.now()}`,
        title_en: extractedData.title_en || file.name,
        title_ar: extractedData.title_ar || 'نص مخصص',
        author: extractedData.author || 'Unknown',
        cover_image: 'https://placehold.co/400x600/10b981/ffffff?text=PDF', // Placeholder for custom
        nodes: extractedData.nodes.map((n: any, idx: number) => ({
          id: `node-${idx}`,
          order: idx + 1,
          text_arabic: n.text_arabic,
          text_english: n.text_english || '',
          node_type: n.node_type || 'line'
        }))
      };

      setBooks(prev => [...prev, newBook]);
      setSelectedBook(newBook); // Auto-open
    ...
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 p-6 font-sans">
        ...
        <main className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Upload Card */}
          <label className={`
            relative bg-white border-2 border-dashed border-slate-300 rounded-xl p-6 
            hover:border-emerald-500 hover:bg-emerald-50/30 transition-all cursor-pointer
            flex flex-col items-center justify-center text-center gap-4 min-h-[160px]
          ...
          `}>
             ...
             {isProcessing ? (
               <>
                 ...
                 <div>
                   <h3 className="text-lg font-semibold text-slate-800">Processing PDF</h3>
                   <p className="text-sm text-slate-500 mt-1">{processingStatus}</p>
                 </div>
               </>
             ) : (
               <>
                 ...
                 <div>
                   <h3 className="text-lg font-semibold text-slate-800">Import Custom PDF</h3>
                   <p className="text-sm text-slate-500 mt-1">Upload a Mutn or Surah (PDF) to generate a lesson.</p>
                 </div>
               </>
             )}
          </label>
          ...
          {books.map(book => (
            ...
            >
              ...
              <img 
                ...
                alt={book.title_en} 
                className="w-20 h-28 object-cover rounded shadow-md z-10 bg-slate-200"
              />
              
              <div className="z-10 flex-1">
                <h3 className="text-xl font-bold font-serif mb-1 group-hover:text-emerald-700 transition-colors line-clamp-2">{book.title_en}</h3>
                <p className="font-arabic text-lg text-slate-600 mb-2 line-clamp-1">{book.title_ar}</p>
      ...
      <div className="flex-1 h-full overflow-y-auto bg-white border-r border-slate-200 relative">
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-slate-100 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2 bg-slate-100 p-1 rounded-lg">
            ...
            <button 
              onClick={() => setBlurLevel('full')}
              className={`p-1.5 rounded ${blurLevel === 'full' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-400'}`}
              title="Hide All"
            >
              <div className="relative">
                 <EyeOff size={16} />
                 <div className="absolute inset-0 bg-slate-400/20 backdrop-blur-[1px] rounded"></div>
              </div>
            </button>
          </div>
        </div>
